...
def explain_prediction(class_name, cam):
    ...
    # divide CAM into 3 horizontal regions to check where it focused
    top = cam_np[:h//3, :].mean()
...
def detect_cam_region(cam):
    ...
    # get coordinates of highest activation
...
def main():
    ...
    if not classes_path.exists():
        print("ERROR: classes.txt not found.\nLooked for:", classes_path)
        print("Run your CIFAR-10 training first (train_cifar10.py).")
        sys.exit(1)
    if not ckpt_path.exists():
        print("ERROR: best.pth not found.\nLooked for:", ckpt_path)
        print("Run your CIFAR-10 training first (train_cifar10.py).")
        sys.exit(1)
    ...
    print("Preparing CIFAR-10 test set (may download on first run)...")
    test_ds = datasets.CIFAR10(root=DATA_DIR, train=False, download=True, transform=tf)
    print("Test images:", len(test_ds))

    #show N random samples with Grad-CAM
    N = min(N_SAMPLES, len(test_ds))
    print(f"Visualizing {N} random images...")
    plt.figure(figsize=(10, 2*N+2))
    for i in range(N):
        ...
        cam = gc(logits, class_idx=pred_id)     # [1,1,h,w]
        cam_up = F.interpolate(cam, size=img.shape[1:], mode='bilinear', align_corners=False)[0,0]
        cam_up = cam_up.clamp(0,1).cpu()
        ...
        plt.subplot(N, 2, 2*i+1)
        plt.imshow(disp); plt.axis('off')
        plt.title(f"GT: {class_names[true_id]}\nPred: {class_names[pred_id]} ({pred_p:.2f})", fontsize=9)
