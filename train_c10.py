...
DATA_DIR   = r"D:\%%SLU\Sem 3\AI-Capstone\MVIT\data"
OUT_DIR    = "MVIT_C10"
...
WARMUP_E   = 5
LABEL_SMOOTH = 0.1
FREEZE_HEADONLY_E = 3   #trainning only the head first, then unfreeze
...
def train_one_epoch(model, loader, optimizer, device, scaler, label_smoothing, writer, step_offset):
    model.train()
...
def main():
    ...
    if not metrics_csv.exists():
        with open(metrics_csv, "w", newline="") as f:
            csv.writer(f).writerow(["epoch","train_loss","train_acc1","val_loss","val_acc1","val_acc5"])
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        tr_loss, tr_acc1 = train_one_epoch(
            model, train_loader, optimizer, device, scaler, LABEL_SMOOTH, writer, step_offset=(epoch-1)*len(train_loader))
        for _ in range(len(train_loader)): sched.step()

        vl_loss, vl_acc1, vl_acc5 = validate(model, val_loader, device, writer, epoch)

        ckpt = {"model": model.state_dict(), "epoch": epoch, "best_val_acc1": max(best_acc1, vl_acc1), "class_names": class_names}
        ...
        with open(metrics_csv, "a", newline="") as f:
            csv.writer(f).writerow([epoch, f"{tr_loss:.6f}", f"{tr_acc1:.4f}", f"{vl_loss:.6f}", f"{vl_acc1:.4f}", f"{vl_acc5:.4f}"])

    writer.close()
    print("\nDone! Check MVIT_C10/ for checkpoints, logs, and metrics.\nOpen TensorBoard with:  tensorboard --logdir MVIT_C10/tb")
